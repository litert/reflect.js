...
describe('static getter', function() {
    ...
    it('Should get the metadata value as set', function() {
        ...
        class Test {
            ...
            public static get sg1(): number { return 123; }
        }

        $Assert.strictEqual($Reflect.getStaticAccessorMetadata(Test, 'sg1', key), rV);
    });
    ...
    it('Should overwrite the previous metadata by default', function() {
        ...
        $Assert.strictEqual($Reflect.getStaticAccessorMetadata(Test, 'sg1', key), rV2);
    });
    ...
    it('Should get array when the key is set to be `multiple` on duplicated', function() {
        ...
        $Assert.strictEqual(
            $Reflect.getStaticAccessorMetadata(Test, 'sg1', k1).reduce((p: number, q: number) => p + q, 0) + $Reflect.getStaticAccessorMetadata(Test, 'sg1', k2),
            rVs[2] + rVs[0] + rVs[1]
        );
    });
    ...
    it('Should get undefined when metadata deleted', function() {
        ...
        $Reflect.deleteStaticAccessorMetadata(Test, 'sg1', k1);

        $Assert.strictEqual($Reflect.getStaticAccessorMetadata(Test, 'sg1', k1), undefined);
    });
    ...
    it('Should get all keys of metadata', function() {
        ...
        const keys = $Reflect.getStaticAccessorMetadataKeys(Test, 'sg1');
    ...
    it('Should not get deleted keys', function() {
        ...
        $Reflect.deleteStaticAccessorMetadata(Test, 'sg1', k2);
