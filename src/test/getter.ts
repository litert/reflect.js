...
describe('getter', function() {
    ...
    it('Should get the metadata value as set', function() {
        ...
        class Test {
            ...
            public get g1(): number { return 123; }
        }

        $Assert.strictEqual($Reflect.getAccessorMetadata(Test, 'g1', key), rV);
    });
    ...
    it('Should overwrite the previous metadata by default', function() {
        ...
        $Assert.strictEqual($Reflect.getAccessorMetadata(Test, 'g1', key), rV2);
    });
    ...
    it('Should get array when the key is set to be `multiple` on duplicated', function() {
        ...
        $Assert.strictEqual(
            $Reflect.getAccessorMetadata(Test, 'g1', k1).reduce((p: number, q: number) => p + q, 0) + $Reflect.getAccessorMetadata(Test, 'g1', k2),
            rVs[2] + rVs[0] + rVs[1]
        );
    });
    ...
    it('Should get undefined when metadata deleted', function() {
        ...
        $Reflect.deleteAccessorMetadata(Test, 'g1', k1);

        $Assert.strictEqual($Reflect.getAccessorMetadata(Test, 'g1', k1), undefined);
    });
    ...
    it('Should get all keys of metadata', function() {
        ...
        const keys = $Reflect.getAccessorMetadataKeys(Test, 'g1');
    ...
    it('Should not get deleted keys', function() {
        ...
        $Reflect.deleteAccessorMetadata(Test, 'g1', k2);
